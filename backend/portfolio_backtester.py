...
def run_portfolio(strategy_ids, initial_bankroll=1000.0, risk_method='fixed_1'):
    ...
    for s in selected_strategies:
        ...
        res = bt.run(
            ...
            end_date=p.get('end_date', datetime.today().strftime('%Y-%m-%d')),
            market=p.get('market'),
            value_threshold=p.get('value_threshold', 1.05),
            initial_bankroll=1000.0, # Dummy initial bankroll just to get raw bets
            staking_rule='fixed',
            stake_value=10.0,
            odds_source=p.get('odds_source', 'B365')
        )
        
        if "error" not in res and 'bets_record' in res:
            for b in res['bets_record']:
                b['strategy_id'] = s['id']
