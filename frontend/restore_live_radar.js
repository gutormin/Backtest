...
window.renderLiveSteamTable = function(results) {
    ...
    results.forEach((item, idx) => {
        // Normalize market
        let normMkt = item.market;
        if (item.market === 'HOME') normMkt = 'Home';
        else if (item.market === 'AWAY') normMkt = 'Away';
        else if (item.market === 'DRAW') normMkt = 'Draw';
        else if (item.market === 'OVER 2.5' || item.market === 'OVER_2.5') normMkt = 'Over 2.5';
        else if (item.market === 'UNDER 2.5' || item.market === 'UNDER_2.5') normMkt = 'Under 2.5';
        
        const cacheKey = `${item.league_code}|${normMkt.toLowerCase()}`;
